...
>
> 📊 可视化架构图：[notification-system.html](notification-system.html)
...
- 不做 SLA 承诺 — V1 先跑通核心链路，积累运行数据后再定义 SLA（后续迭代）
...
#### 事件类型注册表（event_type → provider 映射）
...
### 3.2 方案：SELECT ... FOR UPDATE SKIP LOCKED
...
| 版本 | 架构 | 触发条件 |
|---|---|---|
| V1（当前） | DB Outbox + 1s 轮询 Worker + Adapter | 起步方案，只依赖数据库 |
| V2 | 引入 MQ (RabbitMQ)，DB 降级为持久化存档 | QPS > 500 或 DB 压力大 |
| V3 | 配置化模板引擎 + 管理后台 + SLA 定义 | 供应商数量 > 10，运行数据充足 |
| V4 | 租户隔离 + 限流 + SLA 分级 | 多租户需求 |
...
| 阶段 | AI 扮演的角色 | 实际价值 |
|---|---|---|
| 需求分析 | 产品经理 | 追问"不做什么"比"做什么"更重要，帮助划清系统边界 |
| 架构设计 | CTO / 架构师 | 提出 Kafka、Redis、事件溯源等方案，再辩证分析为什么 V1 不需要 |
| 方案取舍 | 技术决策顾问 | 每个"不采纳"都有理由，不是拍脑袋而是经过正反论证 |
| 细节推演 | 高级工程师 | 死信分级、熔断策略、SKIP LOCKED 并发控制等关键细节的深入推敲 |
| 文档输出 | 技术写作者 | 将讨论过程中的决策沉淀为结构化设计文档 |
...
4. **记录取舍理由** — 每个决策的"为什么不"和"为什么是"同样重要（见第 9 节）
...
这个对话过程就是本文档第 4 节（重试与死信策略）和第 9 节（AI 建议取舍）的设计来源。**AI 没有替代任何决策，但它让每个决策都经过了更充分的论证。**

---

## 附录 A：系统架构总览

> 交互式可视化版本：浏览器打开 [`notification-system.html`](notification-system.html)
...
│  ┌───────────────┐  │    POST /notifications                   │  ┌───────────────┐  │
│  │  订单模块     │──┼──┐                                       │  │ 库存系统 API  │  │
│  │  order.*      │  │  │                                       │  │ POST /inventory│  │
│  └───────────────┘  │  │                                       │  └───────────────┘  │
│  ┌───────────────┐  │  │   ┌─────────────────────────────┐     │  ┌───────────────┐  │
│  │  支付模块     │──┼──┤   │   Notification Service      │     │  │ CRM 供应商    │  │
│  │  payment.*    │  │  │   │   (V1 只依赖数据库)          │     │  │ PUT /contacts │  │
│  └───────────────┘  │  │   │                             │     │  └───────────────┘  │
│  ┌───────────────┐  │  │   │  ┌───────────────────────┐  │     │  ┌───────────────┐  │
│  │  广告模块     │──┼──┼──▶│  │ API 接入层            │  │     │  │ 广告平台 API  │  │
│  │  ad.*         │  │  │   │  │ 校验·路由·写DB·202    │  │     │  │ POST /callback│  │
│  └───────────────┘  │  │   │  └──────────┬────────────┘  │     │  └───────────────┘  │
│  ┌───────────────┐  │  │   │  ┌──────────▼────────────┐  │     │  ┌───────────────┐  │
│  │  CRM 模块     │──┼──┘   │  │ Outbox 持久层         │  │     │  │ 其他第三方    │  │
│  │  crm.*        │  │      │  │ notifications 表      │  │     │  │ 各自协议      │  │
│  └───────────────┘  │      │  │ 状态机·SKIP LOCKED    │  │     │  └───────────────┘  │
│                     │      │  └──────────┬────────────┘  │     │                     │
│                     │      │  ┌──────────▼────────────┐  │     │                     │
│   ◀── 终态回调 ◀──┼──────│  │ Delivery Worker       │──┼────▶│                     │
│   delivered /       │      │  │ per-provider 隔离     │  │     │                     │
│   handling /        │      │  │ 1s轮询·退避·熔断     │  │     │                     │
│   failed            │      │  └──────────┬────────────┘  │     │                     │
│                     │      │  ┌──────────▼────────────┐  │     │                     │
│                     │      │  │ Provider Adapter      │  │     │                     │
...
```
业务模块 POST /notifications
(event_type + biz_id + payload + callback_url)
                    │
                    ▼
    ┌───────────────────────────────┐
    │ 校验 event_type 是否已注册    │
    ...
    │ per-provider · status→sending │
    └───────────────┬───────────────┘
                    │
                    ▼
    ┌───────────────────────────────┐
    │ Adapter 构建请求 → 调外部 API │
    └───────────────┬───────────────┘
                    │
                    ▼
            ┌───────────────┐
...
```
                         ...
                         ▼                                              │
┌─────────┐    ┌─────────┐    ┌───────────┐    ┌─────────────┐    ┌────┴─────┐
│ pending │───▶│ sending │───▶│ delivered │    │ dead_letter │───▶│ handling │
└─────────┘    └────┬────┘    └───────────┘    └──────┬──────┘    └────┬─────┘
                    │                                 │                │
                    ▼                                 │                ├───▶ delivered
               ┌──────────┐                           │                │
...
终态：
  delivered  ── 投递成功，外部 API 确认接收
  handling   ── 死信跟进中，通知系统团队协调第三方
  failed     ── 永久失败，业务方自行决定是否补偿
```
...
```
Worker 分组（per-provider 隔离）：

┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐
│ worker-group-ad  │  │ worker-group-crm │  │ worker-group-inv │
│ provider=        │  │ provider=        │  │ provider=        │
│  ad_platform     │  │  crm_vendor      │  │  inventory_system│
│                  │  │                  │  │                  │
│ A 挂了只影响这里 │  │ B 正常投递       │  │ C 正常投递       │
└──────────────────┘  └──────────────────┘  └──────────────────┘

熔断策略：
  同一 provider 连续失败 10 次
    → 暂停该 provider 5 分钟
    → 告警通知人工
    → 避免轰击已宕机的服务
```
